...
def generate_text(lints_list: list, hardcoded_part: str):
    lints_list.sort(key=lambda item: (not item[1], item[0]))
    beautiful_list = []
    for i in lints_list:
        beautiful_list.append("    Lint(\"{}\", {})".format(i[0], str(i[1]).lower()))
    text = hardcoded_part + ",\n".join(beautiful_list) + "\n)"
    return text
...
class LintsUpdater(UpdaterBase):
    ...
    def _update_locally(self):
        text_rustc = generate_text(get_rustc_lints(), HARDCODED_PART_RUSTC)
        with open(RUSTC_LINTS_PATH, "w") as f:
            f.write(text_rustc)

        text_clippy = generate_text(get_clippy_lints(), HARDCODED_PART_CLIPPY)
