...
ISSUE = 'https://github.com/neonaot/intellij-rust/issues/17'
...
def parse_error_from_rust_lang(error: str):
    ...
    no_emitted = 'this error code is no longer emitted by the compiler' in error
    ...
    return no_emitted, error_id, error_description
...
def get_text():
    ...
    if not_found_in_issue:
        text += "These errors were not found in the meta issue https://github.com/neonaot/intellij-rust/issues/17:\n"
        text += generate_list(not_found_in_issue, rust_errors_with_description)
        text += '\n'

    outdated = issue_errors.intersection(outdated_rust_errors)
    if outdated:
        text += "These errors are no longer emitted by the compiler:\n"
        text += generate_list(outdated, outdated_rust_errors_with_description)
        text += '\n'

    not_found = issue_errors.difference(rust_errors).difference(outdated)
    if not_found:
        text += "These errors were not found on the page {}, " \
...
if __name__ == '__main__':
    ...
    if text is None:
        ...
        text = 'Something went wrong at runtime, please check the scripts.'
        print("logs: problems")
    elif len(text) == 0:
